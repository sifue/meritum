...
module.exports = (robot) => {
    ...
    robot.hear(/^mhelp>$/i, (res) => {
        res.send('*プロジェクトmeritum* とは、 *めりたん* と *称号* を集めるプロジェクト。' +
    ...
    robot.hear(/^mlogin>$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            if (countLoginBonus === 1) {
                // 取得済み
                yield t.commit();
                res.send(`<@${slackId}>ちゃんは、既に今日のログインボーナスをゲット済みだよ。`);
            }
            else {
                // 付与へ
                // アカウントがない場合には作り、100めりたん付与、ログインボーナス実績を追加
                const oldAccount = yield accounts_1.Account.findByPk(slackId, { transaction: t });
                let meritum = 0;
                if (!oldAccount) {
                ...
                yield loginBonuses_1.LoginBonus.create({
                    ...
                    { transaction: t };
                yield t.commit();
                res.send(`<@${slackId}>ちゃんに、ログインボーナスとして *${LOGIN_BONUS_MERITUN}めりたん* をプレゼント。これで *${meritum}めりたん* になったよ。`);
            }
        }
        catch (e) {
    ...
    // ジャンケン
    robot.hear(/^mjanken> (グー|チョキ|パー) (\d+)$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const user = res.message.user;
        const slackId = user.id;
        const name = user.name;
        const realName = user.real_name;
        ...
        if (bet > MAX_JANKEN_BET) {
            res.send(`*${MAX_JANKEN_BET}めりたん* より大きい数をかけてジャンケンすることは禁止されているよ。`);
            return;
        }
        if (bet <= 0) {
            res.send('*1めりたん* より小さな数の *めりたん* をかけることはできないよ。');
            return;
        }
        const t = yield sequelizeLoader_1.database.transaction();
        try {
            ...
            if (!botAccount) {
                ...
                // ベット分持っていない場合、終了
                res.send(`<@${slackBot.self.id}>は *${bet}めりたん* をもっていないよ。`);
                yield t.commit();
                return;
            }
            ...
            // 相手がベットできるかチェック
            let account = yield accounts_1.Account.findByPk(slackId, { transaction: t });
            if (!account) {
                // アカウントがない場合作る
                const meritum = 0;
                yield accounts_1.Account.create({
                    ...
                    { transaction: t };
                account = yield accounts_1.Account.findByPk(slackId, { transaction: t });
            }
            else if (account.meritum < bet) {
                // ベット分持っていない場合、終了
                res.send(`<@${slackId}>ちゃんは *${bet}めりたん* がないからジャンケンできないよ。`);
                yield t.commit();
                return;
            }
            ...
            if (botHand === hand) {
                res.send(`ジャンケン！ ${botHand}！... *あいこ* だね。またチャレンジしてね。`);
                yield t.commit();
                return;
            }
            const isBotWon = (botHand === 'グー' && hand === 'チョキ') ||
                (botHand === 'チョキ' && hand === 'パー') ||
                (botHand === 'パー' && hand === 'グー');
            if (isBotWon) {
                // 負け処理
                yield accounts_1.Account.update({ meritum: account.meritum - bet }, {
                    where: {
                        slackId: slackId
                    },
                    transaction: t
                });
                yield accounts_1.Account.update({ meritum: botAccount.meritum + bet }, {
                    where: {
                        slackId: slackBot.self.id
                    },
                    transaction: t
                });
                res.send(`ジャンケン！ ${botHand}！...<@${slackId}>ちゃんの *負け* だよ。 *${bet}めりたん* もらうね。これで *${account.meritum -
                    bet}めりたん* になったよ。`);
            }
            else {
                // 勝ち処理
                ...
                res.send(`ジャンケン！ ${botHand}！...<@${slackId}>ちゃんの *勝ち* だよ。 *${bet}めりたん* をあげるね。これで *${account.meritum +
                    bet}めりたん* になったよ。`);
            }
            yield t.commit();
        }
    ...
    // ユーザーとめりたんを賭けてジャンケン
    robot.hear(/^muj> (.+) (\d+)/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        ...
        try {
            ...
            if (opponentAccount.meritum < sendMeritum) {
                // 賭けるめりたんを持っていない場合、終了
                res.send(`<@${opponentSlackId}>は、賭けるための *${sendMeritum}めりたん* をもってないみたいだよ。`);
                yield t.commit();
                return;
            }
            ...
            const displayName = slack.profile.display_name;
            let myAccount = yield accounts_1.Account.findByPk(mySlackId, { transaction: t });
            if (!myAccount) {
                // アカウントがない場合作る
                yield accounts_1.Account.create({
                    slackId: mySlackId,
                    ...
                    { transaction: t };
                myAccount = yield accounts_1.Account.findByPk(mySlackId, { transaction: t });
            }
            // アカウントがない場合に作成してもまだないなら終了
            if (!myAccount) {
            ...
            if (myAccount.meritum < sendMeritum) {
                // 送るめりたんを持っていない場合、終了
                res.send(`<@${mySlackId}>は、賭けるための *${sendMeritum}めりたん* をもってないみたいだよ。`);
                yield t.commit();
                return;
            }
            ...
            const timestamp = (_a = chatPostMessageResponse.message) === null || _a === void 0 ? void 0 : _a.ts;
            ...
            yield web.reactions.add({ channel, name: 'raised_hand_with_fingers_splayed', timestamp });
            ...
            yield t.commit();
            res.send(`<@${mySlackId}>ちゃんから<@${opponentSlackId}>ちゃんへ *${sendMeritum}めりたん* を賭けたジャンケンの招待を送ったよ。`);
    ...
    // ジャンケンの結果の受け取り
    robot.react((res) => __awaiter(void 0, void 0, void 0, function* () {
        var _b;
        ...
        if (message.type === 'added' &&
            ...
            if (session.status === 'offering') { // 敵の手が決まってない場合
                ...
                const channel = chatPostMessageResponse.channel;
                const timestamp = (_b = chatPostMessageResponse.message) === null || _b === void 0 ? void 0 : _b.ts;
                if (!channel || !timestamp) {
                    return;
                }
                yield web.reactions.add({ channel, name: 'fist', timestamp });
                ...
                try {
                    ...
                    if (!myAccount || myAccount.meritum < session.sendMeritum) {
                        yield web.chat.postMessage({
                            channel: session.startChannel,
                            text: `<@${myAccount === null || myAccount === void 0 ? void 0 : myAccount.slackId}>ちゃんはには *${session.sendMeritum}めりたん* なかったみたい。`,
                            as_user: true
                        });
                        yield t.commit();
                        return;
                    ...
                    // 相手に賭けに必要なめりたんがチェック
                    let opponentAccount = yield accounts_1.Account.findByPk(session.opponent, { transaction: t });
                    if (!opponentAccount || opponentAccount.meritum < session.sendMeritum) {
                        yield web.chat.postMessage({
                            channel: session.startChannel,
                            text: `<@${opponentAccount === null || opponentAccount === void 0 ? void 0 : opponentAccount.slackId}>ちゃんはには *${session.sendMeritum}めりたん* なかったみたい。`,
                            as_user: true
                        });
                        yield t.commit();
                        return;
                    ...
                    const isMyWin = (myHand === 'fist' && session.opponentHand === 'v') ||
                    ...
                    if (isMyWin) {
                        // 勝ち処理
                        yield accounts_1.Account.update({ meritum: opponentAccount.meritum - session.sendMeritum }, {
                            where: {
                                slackId: session.opponent
                            },
                            transaction: t
                        });
                        yield accounts_1.Account.update({ meritum: myAccount.meritum + session.sendMeritum }, {
                            where: {
                                slackId: mySlackId
                            },
                            transaction: t
    ...
    // ガチャ
    robot.hear(/^mgacha>$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const user = res.message.user;
        const slackId = user.id;
        const name = user.name;
        const realName = user.real_name;
        ...
        try {
            ...
            if (!account) {
                ...
                // ガチャ費用を持っていない場合、終了
                res.send(`<@${slackId}>ちゃんは、ガチャ費用の *${GACHA_MERITUM}めりたん* がないからガチャできないよ。`);
                yield t.commit();
                return;
            }
            // アカウントがない場合に作成してもまだないなら終了
            ...
            res.send(`称号 *${title}* を手に入れたよ！ 称号数は *${newTitlesStr.length}個* 、全称号は *${newTitlesStr}* 、残り *${newMeritum}めりたん* になったよ。`);
            ...
            if (account.titles.includes(title)) {
                if (Math.random() > 0.8) {
                    ...
                    res.send(`称号 *${title}* はもうあるみたいだから、めりたんbotがもらっちゃうね。 めりたんbotの称号数は *${newBotTitlesStr.length}個* 、全称号は *${newBotTitlesStr}* 、 *${botAccount.meritum}めりたん* になったよ。`);
                }
                else {
                    res.send(`称号 *${title}* はもうあるみたいだね、残念。またチャレンジしてね。`);
                }
            }
            yield t.commit();
        }
    ...
    // 自分のデータ表示
    robot.hear(/^mself>$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const user = res.message.user;
        const slackId = user.id;
        const name = user.name;
        const realName = user.real_name;
        ...
        try {
            ...
            res.send(`<@${slackId}>ちゃんの順位は *第${rank}位* 、 称号数は *${account.numOfTitles}個* 、全称号は *${titlesWithAlt}* 、めりたん数は *${account.meritum}めりたん* だよ。`);
        }
        catch (e) {
    ...
    robot.hear(/mranking>$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const user = res.message.user;
        const slack = user.slack;
        const t = yield sequelizeLoader_1.database.transaction();
        try {
    ...
    // 他人のデータ表示
    robot.hear(/^mrank> (.+)/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const rawText = res.message.rawText;
        if (!rawText) {
    ...
    // 他人にめりたんを送る
    robot.hear(/^msend> (.+) (\d+)/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const rawText = res.message.rawText;
        if (!rawText) {
            res.send('rawTextが正しく取得でいないみたい。');
            return;
        ...
        if (!parsed) {
            res.send('コマンドの形式が `msend> (@ユーザー名) (数値)` ではないみたい。');
            return;
        }
        ...
        const t = yield sequelizeLoader_1.database.transaction();
        try {
            let toAccount = yield accounts_1.Account.findByPk(toSlackId, { transaction: t });
            if (!toAccount) {
                res.send('指定したユーザーはプロジェクトmeritumをやってないみたい。');
                yield t.commit();
                return;
            ...
            let fromAccount = yield accounts_1.Account.findByPk(fromSlackId, { transaction: t });
            if (!fromAccount) {
                // アカウントがない場合作る
                yield accounts_1.Account.create({
                    fromSlackId,
                    ...
                    { transaction: t };
                fromAccount = yield accounts_1.Account.findByPk(fromSlackId, { transaction: t });
            }
            // アカウントがない場合に作成してもまだないなら終了
            if (!fromAccount) {
                res.send('アカウントを作成することができないみたい。');
            ...
            if (fromAccount.meritum < sendMeritum) {
                // 送るめりたんを持っていない場合、終了
                res.send(`<@${fromSlackId}>は、送るための *${sendMeritum}めりたん* をもってないみたいだよ。`);
                yield t.commit();
                return;
            }
            // 送付処理
            yield accounts_1.Account.update({ meritum: fromAccount.meritum - sendMeritum }, {
                where: {
                    slackId: fromSlackId
                },
                transaction: t
            });
            yield accounts_1.Account.update({ meritum: toAccount.meritum + sendMeritum }, {
                where: {
                    slackId: toSlackId
                },
                transaction: t
            ...
            yield t.commit();
            res.send(`<@${fromSlackId}>から<@${toSlackId}>に *${sendMeritum}めりたん* を送って、<@${fromSlackId}>は *${fromAccount.meritum -
    ...
    // 運命のおみくじ
    robot.hear(/^mmikuji>$/i, (res) => __awaiter(void 0, void 0, void 0, function* () {
        const user = res.message.user;
        const slackId = user.id;
        const name = user.name;
        const realName = user.real_name;
        ...
        try {
            ...
            if (countOmikuji === 1) {
                ...
                if (!botAccount) {
                    ...
                    // 最大景品分持っていない場合、終了
                    res.send(`<@${slackBot.self.id}>はおみくじを用意できなかったみたい。`);
                    yield t.commit();
                    return;
                }
                // ボットアカウントがない場合に作成してもまだないなら終了
                ...
                if (!account) {
                    ...
                    // おみくじ代分持っていない場合、終了
                    res.send(`<@${slackId}>ちゃんは *${OMIKUJI_MERITUM}めりたん* ないからおみくじ引けないよ。`);
                    yield t.commit();
                    return;
                }
                // アカウントがない場合に作成してもまだないなら終了
                ...
                yield t.commit();
                if (prizeMeritum === 0) {
                    res.send(`<@${slackId}>ちゃんの今日の運勢は... *${prize}* だよ！ 景品はないみたい。`);
                }
                else {
                    res.send(`<@${slackId}>ちゃんの今日の運勢は... *${prize}* だよ！ 景品に *${prizeMeritum}めりたん* あげるね。`);
                }
            }
        }
        catch (e) {
